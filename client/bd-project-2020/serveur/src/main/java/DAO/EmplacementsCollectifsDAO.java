...
import mesClassesMetier.EmplacementsCollectifs;
 ...
 * @author bouaziz
 */
public class EmplacementsCollectifsDAO extends SQLAble implements LogementInterface{
    ...
    public ArrayList<Logement> getLogement(String iDFestival, String dateDF, String dateFF, String idHebergement) {
        ...
        try {
            ...
            EmplacementsCollectifs logementCampingDsipo;
        ...
        String query = "SELECT numLogement, tarif, idHebergement FROM ( select * from LesEmplacementsCollectifs natural join LesVillageVacances where idHebergement = "+idH+" ) A NATURAL JOIN (select numLogement, count(*) from LesPeriodeDeDisponibilites where (dateDispo >= TO_DATE("+dateDF+",'DD-MM-YYYY') and dateDispo <= TO_DATE("+dateFF+",'DD-MM-YY')) group by (numLogement) having count(*) >= TO_DATE("+dateFF+",'DD-MM-YY') -  TO_DATE("+dateDF+",'DD-MM-YY') + 1) B";
            ...
            while (resultats.next()) {
                ...
                // j'instancie la classe festival et je l'ajoute a ma liste de festivale
                           logementCampingDsipo = new EmplacementsCollectifs(numLogement, tarif, idHebergementInt);
