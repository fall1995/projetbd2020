...
export class AuthService {
  ...
  constructor(private http: HttpClient) { }
    ...
    authentificate(params: {[key: string]: string}): Promise<HttpResponse<string>> {
        ...
        return this.http.post( `${this.serverUrl}/api/authentification`, P, {
            observe: 'response',
            responseType: 'text',
    ...
    async getUser(id : string): Promise<User> {
        return new Promise<User>(((resolve, reject) => {
            this.http.get(`${this.serverUrl}/api/authentification?idClient=${id}`, {responseType: 'text'}).toPromise().then(
                res => {
    ...
    modification(params: {[key: string]: string}): Promise<HttpResponse<string>> {
        const P = new HttpParams( {fromObject: params} );
        return this.http.put( `${this.serverUrl}/api/authentification`, P, {
            observe: 'response',
            responseType: 'text',
            headers: {'content-type': 'application/x-www-form-urlencoded'}
        }).toPromise();
